...
def is_mail(text, login_mail=None):
    if login_mail:
        return True if re.match(r'[0-9a-zA-Z_]{0,19}@%s' % login_mail, text) else False
    p = re.compile(r"[^@]+@[^@]+\.[^@]+")
    # if re.match(r'^[0-9a-zA-Z_]{0,19}@[0-9a-zA-Z]{1,13}\.[com,cn,net]{1,3}$', text):
    return True if p.match(text) else False
    ...
    def __init__(self, key, **conf):
        if not conf:
            ...
            conf = dict(host=__redis_info.get(const.RD_HOST_KEY), port=__redis_info.get(const.RD_PORT_KEY, 6379),
                        db=__redis_info.get(const.RD_DB_KEY, 0), password=__redis_info.get(const.RD_PASSWORD_KEY, None), auth=__redis_info.get(const.RD_AUTH_KEY))

        if conf.get('auth'):
            self.rdcon = redis.Redis(host=conf.get('host'), port=conf.get('port'), password=conf.get('password'),
                                     db=conf.get('db', 0))
        else:
            self.rdcon = redis.Redis(host=conf.get('host'), port=conf.get('port'),
                                     db=conf.get('db', 0))
        self._lock = 0
