...
class MyCryptV3:
    ...
    def load_private_key(cls, private_path=None, private_info=None):
        if not private_path and not private_info:
            raise ValueError('两个参数不能都为空')
    ...
    def load_public_key(cls, public_path=None, public_info=None):
        if not public_path and not public_info:
            raise ValueError('两个参数不能都为空')
    ...
    def to_encrypt(cls, message, public_path=None, public_key=None, be_base64=True):
        ...
        if be_base64:
            return base64.b64encode(cryto_msg).decode("utf-8")  # 将加密文本转化为 base64 编码
